 ...
 */
static uint8_t print_char(uint8_t x, uint8_t y, char character, const struct font *fnt,
			  uint8_t scale);
 ...
 * @param scale Escala de impressão.
 */
static inline void draw_byte_v(uint8_t top, uint8_t left, uint8_t byte, uint8_t scale);
 ...
 * @param scale Escala de impressão.
 */
static inline void draw_byte_h(uint8_t top, uint8_t left, uint8_t byte, uint8_t scale);
...
static struct display_control {
	...
	const uint8_t marker[sizeof(
		CONFIG_VIRTUAL_DISPLAY_MARKER_STR)]; /**< Marcador de início do buffer de vídeo. */
	uint8_t screen_buffer[SCREEN_BUFFER_BYTES];  /**< Segundo buffer de vídeo, 128x64
...
void display_draw_line(uint8_t origin_x, uint8_t origin_y, const uint8_t end_x, const uint8_t end_y)
{
...
void display_draw_rect(uint8_t top, uint8_t left, const uint8_t height, const uint8_t width,
		       const bool filled)
{
...
void display_draw_circle(const uint8_t center_x, const uint8_t center_y, const uint8_t radius)
{
...
void display_draw_bitmap(const uint8_t *bmp, const uint8_t top, const uint8_t left,
			 const uint8_t width, const uint8_t height, const uint8_t scale)
{
	if (bmp == NULL) {
		return;
	}
...
static void draw_horizontal_line_base(const uint8_t x, const uint8_t y, uint8_t w)
{
...
static void draw_vertical_line_base(const uint8_t x, const uint8_t y, uint8_t h)
{
	uint8_t mask;
	uint8_t mod;
	uint8_t *ptr = &self.draw_buffer[(y >> 3) << 7] + x;
...
static inline void draw_byte_h(const uint8_t top, uint8_t left, uint8_t byte, const uint8_t scale)
{
	for (; byte != 0; byte <<= 1) {
		if (byte & 0x80) {
			for (uint8_t i = 0; i < scale; i++) {
...
static inline void print_bmp_horizontal(const uint8_t *bmp, uint8_t top, const uint8_t left,
					const uint8_t width, const uint8_t height,
					const uint8_t scale)
{
...
static void display_draw_h_line(const uint8_t origin_x, const uint8_t end_x, const uint8_t y)
{
...
static void display_draw_v_line(const uint8_t origin_y, const uint8_t end_y, const uint8_t x)
{
